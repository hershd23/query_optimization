...
public:
    std::string name;
    std::string baseTableName; // NOTE: This field is useful while joining tables.
    FieldType type;
    std::unique_ptr<IntHistogram> intHistogram;
    std::unique_ptr<StringHistogram> stringHistogram;

    Column(const std::string& name, const std::string& baseTableName, const FieldType type) : name(name), baseTableName(baseTableName), type(type) {
    ...
    Column(Column&& other) noexcept
        : name(std::move(other.name)), baseTableName(std::move(other.baseTableName)) , type(other.type),
    ...
    Column& operator=(Column&& other) noexcept {
        if (this != &other) {
            ...
            baseTableName = std::move(other.baseTableName);
            type = other.type;
            intHistogram = std::move(other.intHistogram);
            stringHistogram = std::move(other.stringHistogram);
        }
...
public:
    ...
    double estimateSelectivity(const std::string& columnName, Predicate::Op op, const Field& value) const {
        ...
        if (it == columns.end()) {
            throw std::runtime_error("Column not found (selectivity): in table " + name + ": " + columnName);
        }
